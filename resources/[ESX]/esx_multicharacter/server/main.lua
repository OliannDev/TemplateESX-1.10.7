...
local function SetupCharacters(source)
	...
	local result = MySQL.query.await('SELECT identifier, accounts, job, job_grade, job2, job2_grade, firstname, lastname, dateofbirth, sex, skin, disabled FROM users WHERE identifier LIKE ? LIMIT ?', {identifier, slots})
	local characters
	...
	if result then
		...
		for i = 1, characterCount, 1 do
			...
			local job2, grade2 = v.job2 or 'nojob2', tostring(v.job2_grade)
			...
			if ESX.Jobs2[job2] and ESX.Jobs2[job2].grades[grade2] then
				if job2 ~= 'nojob2' then grade2 = ESX.Jobs2[job2].grades[grade2].label else grade2 = '' end
				job2 = ESX.Jobs2[job2].label
			end

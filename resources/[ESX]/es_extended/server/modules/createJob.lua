...
local function doesJobAndGradesExist(name, grades)
    if not ESX.Jobs[name] or not ESX.Jobs2[name] then
       return false
    end

   for _, grade in ipairs(grades) do
       if not ESX.DoesJobExist(name, grade.grade) or not ESX.DoesJob2Exist(name, grade.grade) then
           return false
       end
   end
...
local function generateTransactionQueries2(name,grades)
    local queries2 = {}
    for _, grade in ipairs(grades) do
        queries2[#queries2+1] = {
...
function ESX.CreateJob2(name, label, grades)
    ...
    MySQL.insert('INSERT IGNORE INTO jobs2 (name, label) VALUES (?, ?)', {name, label}, function(jobId)
        if jobId == nil or jobId == 0 then
            notify("ERROR",currentResourceName, 'Failed to insert job: `%s`', name)
            return
        end

        local queries = generateTransactionQueries2(name, grades)
        ...
        MySQL.transaction(queries, function(results)
            ...
            ESX.Jobs[name] = generateNewJob2Table(name,label,grades)
            notify("SUCCESS",currentResourceName, 'Job created successfully: `%s`', name)
        end)
    end)
